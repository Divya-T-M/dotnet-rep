        ...
        public IActionResult Index(Employee emp, string action, int currentPage = 1, int pageSize = 10)
        {
            ...
            if (employeeData != null)
            {
                ...
                if (!string.IsNullOrEmpty(emp.Language))
                {
                    ...
                    var empDataByState = employeeData.GroupBy(x => x.State).Select((g, index) => new
                    {
                        GroupId = index + 1,
                        State = g.Key,
                        TotalVolVar = (float)g.Sum(x => x.VolVar),
                        TotalVolVar1 = (float)g.Sum(x => x.VolVar1),
                        TotalVolVar2 = (float)g.Sum(x => x.VolVar2),
                        TotalRevVar = (float)g.Sum(x => x.RevVar),
                        TotalRevVar1 = (float)g.Sum(x => x.RevVar1),
                        TotalRevVar2 = (float)g.Sum(x => x.RevVar2),
                        EmployeeData = g.ToList()  
                    }).ToList();
        ...
        private IActionResult DownloadRevenue(IEnumerable<Employee> employees, DateTime StartDate, DateTime EndDate)
        {
            using (MemoryStream memoryStream = new MemoryStream())
            {
                using (var package = new ExcelPackage(memoryStream))
                {
                    ...
                    worksheet.Cells["A1:D1"].Style.Font.Color.SetColor(System.Drawing.Color.DarkBlue);
                    ...
                    worksheet.Cells["E1:F1"].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.LightGreen);
                    worksheet.Cells["E1:F1"].Style.Font.Color.SetColor(System.Drawing.Color.DarkGreen);

                    worksheet.Cells["G1:H1"].Merge = true;
                    worksheet.Cells["G1"].Value = EndDate.ToString("yyyy-MM-dd");
                    worksheet.Cells["G1:H1"].Style.Font.Bold = true;

                    worksheet.Cells["G1:H1"].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                    worksheet.Cells["G1:H1"].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.LightYellow);
                    worksheet.Cells["G1:H1"].Style.Font.Color.SetColor(System.Drawing.Color.Orange);

                    worksheet.Cells["I1:K1"].Merge = true;
                    worksheet.Cells["I1"].Value = "Stater";
                    worksheet.Cells["I1:K1"].Style.Font.Bold = true;

                    worksheet.Cells["I1:K1"].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                    worksheet.Cells["I1:K1"].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.LightCoral);
                    worksheet.Cells["I1:K1"].Style.Font.Color.SetColor(System.Drawing.Color.Red);
                    ...
                    worksheet.Cells["L1:M1"].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.LightGray);
                    worksheet.Cells["L1:M1"].Style.Font.Color.SetColor(System.Drawing.Color.DarkGray);
